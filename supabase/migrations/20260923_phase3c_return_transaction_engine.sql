-- ============================================================================
-- DELIVERE — PHASE 3C / STEP 6.1: CANONICAL RETURN TRANSACTION ENGINE
-- File: supabase/migrations/20260923_phase3c_return_transaction_engine.sql
-- Description: Authoritative reverse logistics transaction layer:
...
-- Mode: Zero data mutation of historical forward legs. Security Definer + service_role only.
-- ============================================================================

BEGIN;

-- ----------------------------------------------------------------------------
-- 1. RPC: execute_initiate_shipment_return
-- Purpose:
--   Instantiates a canonical reverse journey (RETURN leg) safely without modifying
--   historical forward legs, payments, or physical parcel custody.
-- Invariants:
--   - Validates tenant is active and actor is authorized (ADMIN, DISPATCHER, SUPER_ADMIN, MERCHANT, DRIVER).
--   - Replays idempotently when provided with an identical idempotency key.
--   - Locks shipment and current custody to prevent race conditions.
--   - Strictly prevents creating duplicate active RETURN legs for the same shipment.
--   - Dynamically resolves origin & destination endpoints to satisfy check constraints.
--   - Appends a new sequence under lock; links continuation_of_leg_id to last completed/failed leg.
--   - Appends domain event 'RETURN_INITIATED' to public.shipment_events.
--   - Does NOT mutate physical custody or financial snapshots.
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    -- 9. Insert Assignment offer record if driver assigned (Uses structural schema default 0.000 unpriced)
    ...
    -- 10. Append Domain Event (Persisting full material context for idempotency validation)
...
-- ----------------------------------------------------------------------------
-- 2. RPC: execute_confirm_customer_return_pickup
-- Purpose:
--   Executes the physical handoff of a parcel from CUSTOMER to DRIVER for a
--   post-delivery customer return leg.
-- Invariants:
--   - Validates tenant, actor, and driver assignment.
--   - Validates active RETURN leg with origin_is_shipment_customer = true.
--   - Validates current custody is CUSTOMER (is_with_customer = true).
--   - Replays idempotently when provided with an identical idempotency key.
--   - Atomically records custody_event 'REVERSE_HANDOFF' (CUSTOMER -> DRIVER).
--   - Updates shipment_current_custody to DRIVER.
--   - Advances RETURN leg status to IN_TRANSIT.
--   - Appends domain event 'CUSTOMER_RETURN_PICKED_UP' to public.shipment_events.
--   - Does NOT collect COD cash or alter delivery fee snapshots.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.execute_confirm_customer_return_pickup(
    p_tenant_id UUID,
    p_actor_user_id UUID,
    p_shipment_id UUID,
    p_leg_id UUID,
...
BEGIN
    ...
    IF p_idempotency_key IS NOT NULL THEN
        ...
        IF FOUND THEN
            IF v_existing_event.event_type <> 'REVERSE_HANDOFF' OR
               ...
               v_existing_event.from_is_shipment_customer IS NOT TRUE OR
               (p_driver_id IS NOT NULL AND v_existing_event.to_driver_id IS DISTINCT FROM p_driver_id) OR
               v_existing_event.evidence_type IS DISTINCT FROM p_evidence_type OR
               v_existing_event.evidence_reference IS DISTINCT FROM COALESCE(p_evidence_otp, p_evidence_barcode) THEN
                RAISE EXCEPTION 'IDEMPOTENCY_REPLAY_MISMATCH: Idempotency key % already used for different context', p_idempotency_key
                    USING ERRCODE = '23505';
            END IF;
            ...
            RETURN jsonb_build_object(
                ...
                'message', 'Customer return pickup already processed'
            );
        END IF;
    END IF;
    ...
    IF v_leg.leg_type <> 'RETURN' THEN
        RAISE EXCEPTION 'INVALID_LEG_TYPE: Customer return pickup requires RETURN leg, found %', v_leg.leg_type
            USING ERRCODE = '23514';
    END IF;

    IF v_leg.origin_is_shipment_customer IS NOT TRUE THEN
        RAISE EXCEPTION 'ORIGIN_MISMATCH: Customer return pickup requires leg origin to be customer'
            USING ERRCODE = '23514';
    END IF;

    IF v_leg.status IN ('COMPLETED', 'CANCELLED', 'FAILED') THEN
        RAISE EXCEPTION 'INVALID_LEG_STATE: Cannot pick up return leg % in status %', p_leg_id, v_leg.status
            USING ERRCODE = '23514';
    END IF;

    IF v_leg.assigned_driver_id IS NULL THEN
        RAISE EXCEPTION 'DRIVER_NOT_ASSIGNED: Return leg % has no assigned driver', p_leg_id
            USING ERRCODE = '23514';
    END IF;

    IF p_driver_id IS NOT NULL AND p_driver_id IS DISTINCT FROM v_leg.assigned_driver_id THEN
        RAISE EXCEPTION 'DRIVER_MISMATCH: Supplied driver % does not match assigned driver %', p_driver_id, v_leg.assigned_driver_id
            USING ERRCODE = '23514';
    END IF;
    ...
    -- Custody MUST be CUSTOMER
    IF v_custody.shipment_id IS NULL OR v_custody.current_holder_type <> 'CUSTOMER' OR v_custody.is_with_customer IS NOT TRUE THEN
        RAISE EXCEPTION 'CUSTODY_MISMATCH: Parcel is in custody of %, expected CUSTOMER', v_custody.current_holder_type
            USING ERRCODE = '23514';
    END IF;

    v_evidence_ref := COALESCE(p_evidence_otp, p_evidence_barcode);

    -- 5. Atomic Mutations
    -- 5a. Append Custody Event (CUSTOMER -> DRIVER)
    INSERT INTO public.custody_events (
        tenant_id, shipment_id, leg_id, event_type,
        from_is_shipment_customer, to_driver_id,
    ...
    INSERT INTO public.shipment_events (
        ...
        p_tenant_id, p_shipment_id, p_leg_id, 'CUSTOMER_RETURN_PICKED_UP',
        p_actor_user_id, v_actor.role, v_leg.assigned_driver_id,
        v_now, COALESCE(p_idempotency_key, gen_random_uuid()::text),
        jsonb_build_object('evidence_type', p_evidence_type, 'notes', p_notes),
        v_now
    );
...
-- ----------------------------------------------------------------------------
-- 3. RPC: execute_confirm_merchant_return_receipt
-- Purpose:
--   Executes the terminal physical return handoff of a parcel to the MERCHANT branch
--   from DRIVER or FACILITY.
-- Invariants:
--   - Validates tenant, actor, and merchant ownership of the shipment.
--   - Validates active terminal RETURN leg.
--   - Validates current custody is DRIVER or FACILITY.
--   - Enforces destination branch belongs to the shipment's merchant.
--   - Replays idempotently when provided with an identical idempotency key.
--   - Atomically records custody_event 'REVERSE_HANDOFF' (DRIVER/FACILITY -> MERCHANT).
--   - Updates shipment_current_custody to MERCHANT.
--   - Completes the terminal RETURN leg.
--   - Updates shipments.status to 'RETURNED'.
--   - Appends domain event 'MERCHANT_RETURN_COMPLETED' to public.shipment_events.
--   - Does NOT auto-restock inventory, post accounting journal reversals, or alter fee snapshots.
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- SECTION 4: STRICT LEAST-PRIVILEGE SECURITY GRANTS (SERVICE-ROLE ONLY)
-- ============================================================================
...
-- 2. execute_confirm_customer_return_pickup
REVOKE ALL ON FUNCTION public.execute_confirm_customer_return_pickup(UUID, UUID, UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, NUMERIC, NUMERIC, public.custody_evidence_type) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.execute_confirm_customer_return_pickup(UUID, UUID, UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, NUMERIC, NUMERIC, public.custody_evidence_type) FROM anon;
REVOKE ALL ON FUNCTION public.execute_confirm_customer_return_pickup(UUID, UUID, UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, NUMERIC, NUMERIC, public.custody_evidence_type) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.execute_confirm_customer_return_pickup(UUID, UUID, UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, NUMERIC, NUMERIC, public.custody_evidence_type) TO service_role;
