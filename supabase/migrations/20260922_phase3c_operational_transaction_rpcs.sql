-- ============================================================================
-- DELIVERE — PHASE 3C / STEP 1.1: HARDENED OPERATIONAL TRANSACTION RPCS
-- Migration Name: 20260922_phase3c_operational_transaction_rpcs.sql
-- Status: PROPOSAL ONLY (DO NOT APPLY REMOTELY WITHOUT USER AUTHORIZATION)
--
-- Provides true PostgreSQL transaction boundaries (SECURITY DEFINER) for
-- multi-table operational mutations:
...
--
-- HARDENING HIGHLIGHTS (STEP 1.1):
--   - Atomic Outer Transaction Wrapper (BEGIN ... COMMIT)
--   - Strict Security Definer Trust Boundary & fixed search_path = public, pg_temp
--   - Full Actor Validation (existence, active status, tenant isolation, role/facility scope)
--   - Correct Phase 3B Column Alignments (custody_events, shipment_current_custody, shipment_legs)
--   - Authoritative Payment Derivation strictly from locked shipment record
--   - Anti-Spoofing: Rejection of PREPAID requests for COD shipments
--   - Rejection of unauthorized SHORT payments (approval workflow not yet existing)
--   - CLIQ/Wallet creates ZERO driver cash custody; CASH creates strict physical cash record
--   - Concurrency Safety via deterministic lock order: Shipment -> Leg -> Custody
--   - Context-Verified Idempotency Replay
--   - Privilege Hardening: REVOKE from PUBLIC/anon/authenticated; GRANT only to service_role
-- ============================================================================
...
-- Physical Handoff: MERCHANT -> DRIVER
-- Tables mutated atomically:
--   - custody_events (INSERT)
--   - shipment_current_custody (UPSERT / UPDATE with version++)
--   - shipment_legs (UPDATE status to IN_TRANSIT)
--   - shipments (UPDATE status to OUT_FOR_DELIVERY)
--   - shipment_events (INSERT)
-- ----------------------------------------------------------------------------
...
-- Physical Handoff: DRIVER -> FACILITY
-- Tables mutated atomically:
--   - custody_events (INSERT)
--   - shipment_current_custody (UPDATE)
--   - shipment_legs (UPDATE current leg to COMPLETED; advance next leg to READY)
--   - shipment_events (INSERT)
-- ----------------------------------------------------------------------------
...
-- Physical Handoff: FACILITY -> DRIVER
-- INVARIANT: Assignment must already exist; release does NOT assign new drivers.
-- Tables mutated atomically:
--   - custody_events (INSERT)
--   - shipment_current_custody (UPDATE)
--   - shipment_legs (UPDATE status to IN_TRANSIT)
...
BEGIN
    ...
    -- 4. Business Invariant Checks
    -- Invariant: Assignment must already exist prior to physical release
    IF v_leg.assigned_driver_id IS NULL THEN
    ...
    -- Invariant: Target receiving driver must equal canonical assigned driver
    ...
    -- Invariant: Current custody must be the exact source facility
...
-- Physical Handoff: DRIVER -> CUSTOMER
-- Tables mutated atomically:
--   - custody_events (INSERT)
--   - customer_payment_records (INSERT)
--   - driver_cash_collections (INSERT if CASH with paid > 0)
--   - shipment_current_custody (UPDATE to CUSTOMER terminal)
--   - shipment_legs (UPDATE to COMPLETED)
--   - shipments (UPDATE status to DELIVERED; immutable financial snapshots protected)
--   - shipment_events (INSERT)
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    -- Derive payment authority strictly from authoritative database shipment record.
    ...
    ELSE
        -- Fallback check for any other shipment payment types based on cod_amount
    ...
    -- SHORT / OVER Policy
    -- Without an existing approved short-collection workflow, SHORT collections are strictly forbidden.
...
-- Physical Invariant: Failure updates attempt count & leg failure status,
-- but NEVER modifies parcel custody unless physical handoff occurs.
-- Tables mutated atomically:
--   - shipment_legs (UPDATE status to FAILED)
--   - shipments (UPDATE delivery_attempts = delivery_attempts + 1)
--   - shipment_events (INSERT)
-- ----------------------------------------------------------------------------
...
-- Manifest Lifecycle: DRAFT -> SEALED
-- Invariant: Non-empty items check; all legs & shipments compatible and active.
-- Tables mutated atomically:
--   - operational_manifests (UPDATE status to SEALED)
-- ----------------------------------------------------------------------------
